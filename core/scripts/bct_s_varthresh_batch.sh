...
for (( NTOPTERMS=25; NTOPTERMS<=25; NTOPTERMS+=5 ))
do
...
LOGFILE=$LOGDIR/res.s.$NTOPTERMS.txt

echo "Running com.ibm.drl.hbcp.experiments for nterms=$NTOPTERMS"
...
echo "$NTOPTERMS\t$a\t$p\t$r\t$f\t$m"

echo "NTOPTERMS\t$a\t$p\t$r\t$f\t$m" >> tmp
