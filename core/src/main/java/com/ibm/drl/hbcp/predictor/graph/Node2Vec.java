...
public class Node2Vec {
    ...
    boolean addEdge(String src, String dest, float wt) {
        ...
        if (src_node_index == -1) {
            System.out.println(String.format(Locale.ROOT, "Word '%s' OOV...", src));
            return false;
        }
        ...
        if (dst_node_index == -1) {
            System.out.println(String.format(Locale.ROOT, "Word '%s' OOV...", dest));
            return false;
        }
    ...
    void constructGraph(BufferedReader fp) throws IOException {
        ...
        while ((line = fp.readLine()) != null) {
            ...
            count++;
            if (debug_mode > 3)
                System.out.println(String.format(Locale.ROOT, "Read line %d", count));
        }
    }
    ...
    void preComputePathContexts() {
        ...
        for (int i = 0; i < vocab_size; i++) {
            ...
            if (debug_mode > 3)
                System.out.println(String.format(Locale.ROOT, "Precomputed contexts for node %d (%s)", i, vocab[i].word));
        }
    }
    ...
    int adjSampling(int src_node_index, edge[] contextBuff) {
        ...
        len = pIndex;
        if (debug_mode > 2)
            System.out.println(String.format(Locale.ROOT, "#nodes in 2-hop neighborhood = %d", len));
        ...
        for (int i = 0; i < window; i++) {  // draw 'window' samples
            ...
            if (sample <= 0 || keepEdge(multiHopEdgeList[pIndex])) {  // sample==0 means subsampling is off
                ...
                if (debug_mode > 2)
                    System.out.print(String.format(Locale.ROOT, "%s ", vocab[multiHopEdgeList[pIndex].dest.id].word));
            } else {
                if (debug_mode > 2)
                    System.out.print(String.format(Locale.ROOT, "Ignore edge to %s (with subsampling)", vocab[multiHopEdgeList[pIndex].dest.id].word));
            }
        }
...
//            System.out.println(vocab_hash_size);
            ...
            SortVocab();
            if (debug_mode > 2) {
                System.out.println(String.format(Locale.ROOT, "#nodes: %d", vocab_size));
            }
        }
    ...
    void skipgram() {
        ...
        for (int word = 0; word < vocab_size; word++) {
            if (debug_mode > 2) {
                System.out.println(String.format(Locale.ROOT, "Skip-gram iteration for source word %s", vocab[word].word));
    ...
    public boolean train() throws IOException {
        ...
        log.info("Starting training using input graph...");
        LearnVocabFromTrainFile();

        if (output_file == null) { log.error("Graph file not found"); return false; }
        InitNet();

        if (negative > 0) InitUnigramTable();
        log.debug("Unigram table initialized.");

        for (int i=0; i < iter; i++) {
            skipgram();
            log.debug("Iteration " + i + " done.");
        }

        // TODO: the C code was outputting 2 files, binary and string, we only to string
        //output_file_vec = String.format(Locale.ROOT, "%s.vec", output_file);
        ...
        try (BufferedWriter fo = new BufferedWriter(new OutputStreamWriter(output_file_vec))) {
            ...
            fo.write(String.format(Locale.ROOT, "%d %d\n", (long)vocab_size + pt_vocab_words, layer1_size));
            for (int a = 0; a < vocab_size; a++) {
                fo.write(String.format(Locale.ROOT, "%s ", vocab[a].word));
                for (int b = 0; b < layer1_size; b++)
                    fo.write(String.format(Locale.ROOT, "%f ", syn0[a * layer1_size + b]));
                fo.newLine();
            }
    ...
    int ArgPos(String str, int argc, String[] args) {
        for (int a = 0; a < argc; a++) if (args[a].equals(str)) {
            if (a == argc - 1) {
                System.out.println(String.format(Locale.ROOT, "Argument missing for %s", str));
                exit(1);
            }
            return a;
        }
    ...
    public boolean run(int argc, String[] argv) throws IOException {
        ...
        if (window > MAX_CONTEXT_PATH_LEN) {
            System.out.println(String.format(Locale.ROOT, "Window size %d value too large. Truncating the value to %d\n", window, MAX_CONTEXT_PATH_LEN));
            window = MAX_CONTEXT_PATH_LEN;
        }
