...
/**
 * Extract the mentions of psychologists or therapists.
 * 
 * @author charlesj
 *
 */
public class Psychologist extends RegexQueryExtractor<ArmifiedAttributeValuePair>
    implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Psychologist";
    public final static String QUERY_STRING = "psychodramatist therapist psychologist counselor";
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("(psychodramatists?|therapists?|psychologists?|counselors?)")
//            Pattern.compile("(psychodramatists?|therapists?|(clinical )?psychologists?|counselors?)")  // if extracting phrases
    );
    private final Attribute attribute;
    private final Set<Attribute> relevantAttributeSet;

    public Psychologist(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }

    public Psychologist(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        Psychologist extractor = new Psychologist(5);
