...
public class ParsingUtils {
    ...
    public static final Pattern DOUBLE_REGEX = Pattern.compile("((?<![0-9])-?(?:[0-9]+(,[0-9]{3}(?![0-9]))*(?:\\.[0-9]+)?|[0-9]*\\.?[0-9]+))");
    private static final String DOUBLE_PARSING_INCOMPATIBLE_REGEX = ",";
    ...
    public static List<Double> parseAllDoubles(String s) {
        return parseAllDoubleStrings(s).stream()
                // number strings are not necessarily Java-Double.parsable numbers, so we clean them
                .map(rawNumberString -> rawNumberString.replaceAll(DOUBLE_PARSING_INCOMPATIBLE_REGEX, ""))
