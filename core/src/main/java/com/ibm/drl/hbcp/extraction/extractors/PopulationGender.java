...
public class PopulationGender extends TokenMatchingQueryExtractor<ArmifiedAttributeValuePair>
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PopulationGender.class);
    ...
    protected PopulationGender(IndexingMethod indexingMethod, int numberOfTopPassages)
        ...
        relevantAttributesForEvaluation = new HashSet<>();
        for (Gender gender : Gender.values()) {
            try {
                Attribute genderAttribute = Attributes.get().getFromName(gender.longNames);
                relevantAttributesForEvaluation.add(genderAttribute);
            } catch (RuntimeException e) {
                // ignore
                if (LOGGER.isInfoEnabled())
                    LOGGER.info("Gender attribute " + gender.longNames + " is not in the current codeset.");
            }
        }
