...
/**
 * The extractor for 'Individual reasons for attrition' should identify the different reasons for participants to
 * leave a study.  This is usually a complex entity with lists of name-value pairs, e.g., "lost to follow-up": 7%
 * 
 * @author charlesj
 *
 */
public class IndividualReasonsForAttrition extends RegexQueryExtractor<ArmifiedAttributeValuePair> implements IndexBasedAVPExtractor {

    private static final Logger LOGGER = LoggerFactory.getLogger(IndividualReasonsForAttrition.class);

    private static final String ATTRIB_NAME = "Individual reasons for attrition";
    public final static String QUERY_STRING = "lost exclude complete criteria other reason decline die decease withdrew refuse ineligible eligible";

    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("((lost|exclude|complete|criteria|eligible|other|reason|decline|refuse|die|decease|withdrew|no longer).*(\\d?\\d(\\.\\d+)?%?))"),
            Pattern.compile("((\\d?\\d(\\.\\d+)?%?).*(lost|exclude|complete|criteria|eligible|other|reason|decline|refuse|die|decease|withdrew|no longer))")
    );

    private final Attribute attribute;
    private final Set<Attribute> relevantAttributeSet;

    public IndividualReasonsForAttrition(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }

    public IndividualReasonsForAttrition(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
        ...
        PredicateUnarmifiedEvaluator<IndexedDocument, ArmifiedAttributeValuePair> predicateUnarmifiedEvaluator =
                new PredicateUnarmifiedEvaluator<IndexedDocument, ArmifiedAttributeValuePair>() {
                    ...
                    public boolean isCorrect(@NotNull ArmifiedAttributeValuePair predicted, @NotNull ArmifiedAttributeValuePair expected) {
                        ...
                        LOGGER.debug("Expected: {}; Predicted: {}", normExpected, normPredicted);
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        IndividualReasonsForAttrition extractor = new IndividualReasonsForAttrition(5);
