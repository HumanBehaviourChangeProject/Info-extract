...
/**
 * The extractor for 'Encountered Intervention' should identify percentage of encountered interventions, for example,
 * '% attended one or more sessions'.
 * 
 * @author charlesj
 *
 */
public class EncounteredIntervention extends RegexQueryExtractor<ArmifiedAttributeValuePair> implements IndexBasedAVPExtractor {

    protected static final Logger LOGGER = LoggerFactory.getLogger(EncounteredIntervention.class);

    private static final String ATTRIB_NAME = "Encountered Intervention";
    public final static String QUERY_STRING = "attend session use receive";

    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("(\\d?\\d\\.?\\d*%)"),  // look for any number with percent sign
            Pattern.compile("(\\d?\\d\\.?\\d)*.{1,20}(use|attend|recieve)")  // ugly regex, but this is a pretty noisy entity
            );
     ...
     */
    public EncounteredIntervention(int numberOfTopPassages) throws ParseException {
     ...
     * @throws ParseException
     */
    public EncounteredIntervention(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
...
//                        System.out.println("Expected: " + normExpected + "; Predicted: " + normPredicted);
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        EncounteredIntervention extractor = new EncounteredIntervention(5);
