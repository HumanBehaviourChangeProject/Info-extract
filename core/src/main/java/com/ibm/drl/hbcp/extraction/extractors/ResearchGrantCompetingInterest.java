...
/**
 * Extract mentions of research grant competing interests, the mentions are of the institutes, e.g., "National Institutes of Health" or "Cancer Research UK"
 * (see also {@link ResearchGrantFunding})
 * 
 * @author charlesj
 *
 */
public class ResearchGrantCompetingInterest extends RegexQueryExtractor<ArmifiedAttributeValuePair>
    implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Research grant competing interest";
    public final static String QUERY_STRING = "national research cancer institute health foundation department society council university nih";
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("(\bNIH\b)"),
            Pattern.compile("(cancer research uk)", Pattern.CASE_INSENSITIVE),
            Pattern.compile("(([A-Z][a-z]+ )+foundation)", Pattern.CASE_INSENSITIVE),
            Pattern.compile("(university of [A-Z][a-z]+)", Pattern.CASE_INSENSITIVE),
            Pattern.compile("(([A-Z][a-z]+ )+(university|college))", Pattern.CASE_INSENSITIVE),
            Pattern.compile("(([A-Z][a-z]+ )*(health and )?(health |medical )?research council)", Pattern.CASE_INSENSITIVE),
    ...
    public ResearchGrantCompetingInterest(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }

    public ResearchGrantCompetingInterest(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
...
//                System.out.println("Expected: " + expected.getValue() + " ;;; Predicted: " + predicted.getValue());
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        ResearchGrantCompetingInterest extractor = new ResearchGrantCompetingInterest(5);
