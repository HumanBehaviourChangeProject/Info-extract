...
public class Reparser implements PdfAnalysisOutput {
    ...
    private static ReparseDocument parseDocument(File jsonFile) throws IOException {
        File jsonFileInResources = FileUtils.potentiallyGetAsResource(jsonFile);
        ObjectMapper objectMapper = new ObjectMapper()
                .configure(MapperFeature.USE_STD_BEAN_NAMING, true)
                .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        objectMapper.setPropertyNamingStrategy(PropertyNamingStrategy.LOWER_CAMEL_CASE);
        return objectMapper.readValue(jsonFileInResources, ReparseDocument.class);
    }

    public static void main(String[] args) throws IOException {
        Reparser parser = new Reparser(FileUtils.potentiallyGetAsResource(new File("data/All_330_PDFs_extracted/Harris 2010.pdf.json")));
