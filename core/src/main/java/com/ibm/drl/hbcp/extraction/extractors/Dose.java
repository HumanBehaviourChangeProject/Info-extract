...
/**
 * Extract the dosage, i.e., milligrams of nicotine or pieces of gum.
 *
 * Examples: 21mg per day, 21mg, eight pieces of gum initially,
 * six pieces during weeks 5 and 6 and four pieces for the last 2 weeks of treatment
 * 
 * @author charlesj
 *
 */
public class Dose extends RegexQueryExtractor<ArmifiedAttributeValuePair>
    implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Dose";
    public final static String QUERY_STRING = "mg";  //  dosage dose Nicoderm patch gum
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("((\\d+\\.)?\\d+[ -]mg)"),
            Pattern.compile("((\\d+|one|two|three|four|five|six|seven|eight|nine) pieces)")
    );
    private final Attribute attribute;
    private final Set<Attribute> relevantAttributeSet;

    public Dose(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }

    public Dose(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
