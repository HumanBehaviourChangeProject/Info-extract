...
public class WordVec implements Serializable, Clusterable {

	/**
	 * Auto-generated in Eclipse 
	 */
	private static final long serialVersionUID = 8287689727772134064L;
	String word;
    float[] vec;
    float norm;  // L2 norm
    ...
    // useful for context vectors which has a space within the data
    public WordVec(String line, String delimiter) { 
        String[] tokens = line.split(delimiter); // with a tab
        word = tokens[0];
        String vecPart = tokens[1];
        String[] vecComps = vecPart.split(" "); // this is with space
        vec = new float[vecComps.length];
        for (int i = 0; i < vecComps.length; i++)
            vec[i] = Float.parseFloat(vecComps[i]);
    }
    ...
    // allow null vectors in the arguments
    static public WordVec concat(WordVec a, WordVec b) {
        int alen = a==null? 0: a.vec.length;
        int blen = b==null? 0: b.vec.length;
        
        if (alen==0 && blen==0) return null;
        
        WordVec c = new WordVec(alen + blen);
        if (a!=null)
            System.arraycopy(a.vec, 0, c.vec, 0, alen);
        if (b!=null)
            System.arraycopy(b.vec, 0, c.vec, alen, blen);
        return c;
    }
