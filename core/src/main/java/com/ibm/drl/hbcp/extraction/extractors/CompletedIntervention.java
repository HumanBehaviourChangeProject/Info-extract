...
/**
 * The extractor for 'Completed intervention' should identify percentage of completed interventions, for example,
 * '% attended *all* sessions'.
 * 
 * @author charlesj
 *
 */
public class CompletedIntervention extends RegexQueryExtractor<ArmifiedAttributeValuePair> implements IndexBasedAVPExtractor {

    private static final Logger LOGGER = LoggerFactory.getLogger(CompletedIntervention.class);

    private static final String ATTRIB_NAME = "Completed intervention";
    public final static String QUERY_STRING = "attend session use receive complete all";

    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("(\\d?\\d\\.?\\d*%)"),  // look for any number with percent sign
            Pattern.compile("(\\d?\\d\\.?\\d)*.{1,20}(use|attend|recieve|complete|all)")  // ugly regex, but this is a pretty noisy entity
            );
     ...
     */
    public CompletedIntervention(int numberOfTopPassages) throws ParseException {
     ...
     * @throws ParseException
     */
    public CompletedIntervention(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
...
//                        System.out.println("Expected: " + normExpected + "; Predicted: " + normPredicted);
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        CompletedIntervention extractor = new CompletedIntervention(5);
