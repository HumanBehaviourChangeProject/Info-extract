...
/**
 * Extract mentions of healthcare facilities, not doctor-led or hospitals (e.g., clinics)
 * 
 * @author charlesj
 *
 */
public class HealthcareFacility extends RegexQueryExtractor<ArmifiedAttributeValuePair>
    implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Healthcare facility";
    public final static String QUERY_STRING = "clinic \"medical center\" \"primary care\" \"health center\" \"health centre\"";
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("(((smoking cessation|(stop )?smoking|community|medical center|outpatient) )?clinics?)", Pattern.CASE_INSENSITIVE),
            Pattern.compile("((medical|health(care)?) (center|centre))", Pattern.CASE_INSENSITIVE),
            Pattern.compile("(primary care)", Pattern.CASE_INSENSITIVE)
            );
    private final Attribute attribute;
    private final Set<Attribute> relevantAttributeSet;

    public HealthcareFacility(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }

    public HealthcareFacility(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        HealthcareFacility extractor = new HealthcareFacility(10);
