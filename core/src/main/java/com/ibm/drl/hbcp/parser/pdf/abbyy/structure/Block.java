...
public class Block implements com.ibm.drl.hbcp.parser.pdf.Block {
    ...
    public com.ibm.drl.hbcp.parser.pdf.Cell[][] getTableInMatrixForm() {
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            for (int j = 0; j < table[i].length; j++) {
                res[i][j] = convertAbbyyCell(table[i][j], i, j);
            }
        }
        return res;
    }

    public static com.ibm.drl.hbcp.parser.pdf.Cell convertAbbyyCell(Cell cell, int i, int j) {
        return new com.ibm.drl.hbcp.parser.pdf.Cell(
                cell.getValue(),
