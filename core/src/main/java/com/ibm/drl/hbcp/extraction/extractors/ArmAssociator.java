    ...
    public Collection<CandidateInPassage<ArmifiedAttributeValuePair>> associate(
        ...
        for (CandidateInPassage<ArmifiedAttributeValuePair> toBeArmifiedCandidate : predictedCandidates) {
            ...
            if (closestArm !=null) {   
                ...
                armifiedAvp = new ArmifiedAttributeValuePair(
                        toBeArmifiedAvp.getAttribute(),
                        toBeArmifiedAvp.getValue(),
                        docName,
                        closestArm,
                        toBeArmifiedAvp.getContext());
                // mgleize: I think this one is buggy (we don't want to create new arms in an associator)
                //new ArmifiedAttributeValuePair(toBeArmifiedAvp.getAttribute(), toBeArmifiedAvp.getValue(), docName, closestArmName, toBeArmifiedAvp.getContext());
