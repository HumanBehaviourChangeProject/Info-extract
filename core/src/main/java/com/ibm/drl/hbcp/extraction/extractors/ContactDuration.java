...
/**
 * Extract the duration of contacts (of the type 'Schedule').
 * These contacts typically last some number of minutes or hours (e.g., 30 min)
 *
 * @author charlesj
 *
 */
public class ContactDuration extends RegexQueryExtractor<ArmifiedAttributeValuePair>
        implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Contact duration";
    public final static String QUERY_STRING = "hour hr minute min session counsel lasting";
    ...
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("((\\d+.)?\\d+)[- ]+min(ute)?s?\\b"),
            Pattern.compile("((\\d+.)?\\d+)[- ]+h(ou)?rs?\\b"));
    private final Attribute attribute;
    private final Set<Attribute> relevantAttributeSet;

    public ContactDuration(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }

    public ContactDuration(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        ContactDuration durationExtractor = new ContactDuration(IndexingMethod.slidingWindow(50, props),50);
