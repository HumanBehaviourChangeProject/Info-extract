...
public class AbstractDetectPresenceAttribute extends InformationUnit {
    ...
    public AbstractDetectPresenceAttribute(InformationExtractor extractor, String contentFieldName, int code, String attribId) {
    ...
    */
    ...
    public void construct(String window, Query q, float sim) {
        ...
        if (weight>this.simThreshold) {
            this.mostLikelyAnswer = new CandidateAnswer(retrievedText, 0); // second parameter is unused  
        } else {
    ...
    //Overides both the generic and specific values for experiments
    ...
    public void compareWithRef(CodeSetTree gt, InformationUnit predicted, RefComparison rc) {
        ...
        if (predicted != null) 
            predictedText = predicted.mostLikelyAnswer.key;
     ...
     * @return A String denoting the name of this attribute.
     */
    @Override
    ...
    void learnThreshold(CVSplit cvsplit,
        ...
        CodeSetTree gt = extractor.refBuilder.getGroundTruths(this.code);
        ...
        for (int ithresh=1; ithresh<=9; ithresh++) {
            ...
            for (DocIdNamePair dnp: cvsplit.trainDocs) {  // iterate over docs in training set
                ...
                this.compareWithRef(gt, predicted, batchEval);                
            }
