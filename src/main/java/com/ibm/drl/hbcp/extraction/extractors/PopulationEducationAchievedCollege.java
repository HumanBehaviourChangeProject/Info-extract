 ...
 *
 */
public class PopulationEducationAchievedCollege extends RegexQueryExtractor<ArmifiedAttributeValuePair>
    implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Proportion achieved university or college education";
    public final static String QUERY_STRING = "(college OR university OR coll OR uni OR bachelor)";
    // possible terms: matriculation?
    // TODO regular expressions
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("([Cc]ollege|[Uu]niversity)\\D*\\s+((\\d?\\d\\.\\d%?\\s+)+)"), // capture table rows with percentages: first group has college/uni.; second group should match repeated percentages/floats
            Pattern.compile("([Cc]ollege|[Uu]niversity)\\D+((\\d+\\s*\\(\\d?\\d\\.\\d%?\\)\\s+)+)")  // capture percentages in parentheses (usually with actual count, i.e., "College degree 11 (7.5) 8 (6.0)")
            );
    private final Attribute attribute;
    private final Set<Attribute> relevantAttributeSet;

    public PopulationEducationAchievedCollege(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }
 
    public PopulationEducationAchievedCollege(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
...
//        Set<String> retSet = new HashSet<>();
    ...
    public String toString() {
        return "Edu. achieved: College";
    }

    public static void main(String[] args) throws IOException, ParseException {
        Properties props = Props.loadProperties("init.properties");
        PopulationEducationAchievedCollege extractor = new PopulationEducationAchievedCollege(5);
