 ...
 *
 */
public class PopulationProportionMale extends RegexQueryExtractor<ArmifiedAttributeValuePair>
    implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Proportion identifying as male gender";
    public final static String QUERY_STRING = "(gender OR male OR percentage)";
    // possible terms: matriculation?
    // TODO regular expressions
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("([Mm]ale)\\D*\\s+((\\d?\\d\\.\\d%?\\s+)+)"), // capture table rows with percentages: first group has college/uni.; second group should match repeated percentages/floats
            Pattern.compile("([Mm]ale|[Mm]en)\\D+((\\d+\\s*\\(\\d?\\d\\.\\d%?\\)\\s+)+)"),  // capture percentages in parentheses (usually with actual count, i.e., "College degree 11 (7.5) 8 (6.0)")
            Pattern.compile("\\D+((\\d+\\s*\\(\\d?\\d\\.\\d%?\\)\\s+)+)([Mm]ale|[Mm]en)")  // capture percentages in parentheses (usually with actual count, i.e., "College degree 11 (7.5) 8 (6.0)")
            
    );
    private final Attribute attribute;
    private final Set<Attribute> relevantAttributeSet;

    public PopulationProportionMale(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }
 
    public PopulationProportionMale(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
...
//        Set<String> retSet = new HashSet<>();
    ...
    public String toString() {
        return "Male percentage";
    }

    public static void main(String[] args) throws IOException, ParseException {
        Properties props = Props.loadProperties("init.properties");
        PopulationProportionMale extractor = new PopulationProportionMale(5);
