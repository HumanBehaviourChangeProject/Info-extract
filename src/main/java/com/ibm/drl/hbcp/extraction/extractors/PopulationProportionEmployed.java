 ...
 *
 */
public class PopulationProportionEmployed extends RegexQueryExtractor<ArmifiedAttributeValuePair>
    implements IndexBasedAVPExtractor {

    private static final String ATTRIB_NAME = "Proportion employed";
    public final static String QUERY_STRING = "(employed OR work OR full-time OR part-time)";
    // possible terms: matriculation?
    // TODO regular expressions
    private final static List<Pattern> REGEXES = Lists.newArrayList(
            Pattern.compile("([Ee]mployed)\\D*\\s+((\\d?\\d\\.\\d%?\\s+)+)"), // capture table rows with percentages: first group has college/uni.; second group should match repeated percentages/floats
            Pattern.compile("([Ff]ull[- ]time|[Pp]art[- ]time)\\D+((\\d+\\s*\\(\\d?\\d\\.\\d%?\\)\\s+)+)") // capture percentages in parentheses (usually with actual count, i.e., "College degree 11 (7.5) 8 (6.0)")
    ...
    public PopulationProportionEmployed(int numberOfTopPassages) throws ParseException {
        this(IndexingMethod.NONE, numberOfTopPassages);
    }
 
    public PopulationProportionEmployed(IndexingMethod indexingMethod, int numberOfTopPassages) throws ParseException {
...
//        Set<String> retSet = new HashSet<>();
    ...
    public String toString() {
        return "Employed percentage";
    }

    public static void main(String[] args) throws IOException, ParseException {
        Properties props = Props.loadProperties("init.properties");
        PopulationProportionEmployed extractor = new PopulationProportionEmployed(5);
